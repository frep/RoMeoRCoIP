...
#endif
#include "Arduino.h"
#include "../inc/digitalWriteFast/digitalWriteFast.h"
...
uint8_t REDFLY::socketSend(uint8_t socket, char *stream, uint8_t *ip, uint16_t port)
{
  ...
  return cmd(PSTR(CMD_SEND), (char*)buffer, (uint8_t*)stream, size); //x,xxxx,xxx.xxx.xxx.xxx,xxxxx,
}


uint8_t REDFLY::socketSend(uint8_t socket, uint8_t *stream, uint16_t size) //tcp
{
  return socketSend(socket, stream, size, 0, 0);
}


uint8_t REDFLY::socketSendPGM(uint8_t socket, PGM_P stream) //tcp
{
  uint8_t len;
  uint16_t size = strlen_P(stream);
  ...
  uitoa(size, (char*)&buffer[len]);
  //ip+port+data
  strcat_P((char*)buffer, PSTR(",0,0,"));

  return cmd(PSTR(CMD_SEND), (char*)buffer, stream); //x,xxxx,0,0,
}


uint8_t REDFLY::socketSend(uint8_t socket, char *stream) //tcp
{
  return socketSend(socket, (uint8_t*)stream, strlen(stream), 0, 0);
}


uint8_t REDFLY::socketSend(uint8_t socket, int val) //tcp
{
  char buf[8];
             ...
             (buf[7] == 'D')) //RSI_READ
          {
            ...
            for(uint8_t i=0; i<MAX_SOCKETS; i++)
            {
              if((socket_state[i].handle == last_socket) && \
                 (socket_state[i].state  == SOCKET_UDP))
              {
                udp_ip[0] = readwait();
                udp_ip[1] = readwait();
                udp_ip[2] = readwait();
